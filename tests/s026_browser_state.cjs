...
assert.equal(state.edge.instances[second].status, '采集中');
...
// An incomplete run skips forward without losing the collected sample or duration.
state = context.s026PrepareEdgeWorkflow(clone(context.S026_DEFAULT));
context.s026State = state;
state.edge.connected = true;
state.edge.wifiConnected = true;
...
assert.ok(candidateRow(duplicateSource.id).includes('指令包重复'));
assert.ok(candidateRow(duplicateSource.id).includes('disabled'));
assert.ok(candidateRow(duplicateCandidate.id).includes('指令重复'));
...
console.log('Approval multi-select, duplicate imports, package publication, thresholds and collection workflow checks passed');
